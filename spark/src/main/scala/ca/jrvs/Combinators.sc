...
println("Count = " + ls.size)
println("First = " + ls.head)
println("Last = " + ls.last)
println("First five: " + ls.take(5))
println("Last five: " + ls.takeRight(5))
...
numList.flatten.map((i: Int) => i * 2)
numList.flatMap((i: List[Int]) => i.map(_*2))
 ...
 */
println("Sum using sum: " + ls.sum)
println("Sum using reduce: " + ls.reduce((x, y) => x + y))
println("Sum using foldLeft: " + ls.foldLeft(0)((x, y) => x + y))
...
val countryNames = names.map((name: String) => (name, countryMap.getOrElse(name, "n/a")))
 ...
 */
val countryPopulation = countryNames.map(x => x._2).groupBy(country => country).mapValues(_.size)
...
(1 to names2.size toList).zip(names2)
 ...
 */
val source = scala.io.Source.fromFile("/home/tomas/Documents/jarvis_data_eng_TomasRotbauer/spark/src/main/resources/employees.csv")
var data = source.getLines.map(_.split(",")).toList
source.close
